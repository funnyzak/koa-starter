...
class RequestFiles {
  ...
  process(req, opts = {}) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const resolveEnd = () => {
        if (receive_end && file_count === successFiles.length + failFiles.length) {
          return resolve({
      ...
      form
        ...
        .on('file', async (field, file) => {
          if (!opts.limit.mimeType.includes(file.mimetype)) {
            ...
            const saveName = opts.keepOriginName ? `${file.originalFilename}` : `${udidv4()}${suffix}`
            ...
            if (opts.removeTmpFile && opts.isSaveDir && (await fs.existsSync(file.filepath))) {
              await fs.rmSync(file.filepath)
            }
