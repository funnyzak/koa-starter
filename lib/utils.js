...
function koaBodyParse(
  uploadOps = { maxSize: 2 * 1024 * 1024 },
  opts = {}
) {
  return koaBody(
    _.merge(
        ...
        formidable: {
          ...
          filter: ({ name, originalFilename, mimetype }) => {
            return (
              ...
              (mimetype &&
                uploadOps.mimetype.find(
                  (_type) => mimetype === _type
                ))
            )
          },
...
function getNormalTimeStringByDateString(
  ...
  return dtime(dateToTimestamp(dateString.toString())).format(
    formatString
  )
}
...
function msTimeToShow(ms) {
  return humanTime(
    new Date(new Date().getTime() - parseInt(ms))
  ).replace('前', '')
}
...
function parseRequestFiles(ctx) {
  ...
  if (ctx.request.files) {
    ...
    Object.keys(_files).map((key) => {
      let _tmpFiles = !Array.isArray(_files[key])
        ? [_files[key]]
        : _files[key]
      files.push(
        ..._tmpFiles.map((v) => {
