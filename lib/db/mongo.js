...
class MongoDbClient {
   ...
   */
  async ping(db) {
    return (await this.collection('default', null, db)) !== null
  }

  async connect(db) {
    await this.collection('default', null, db)
  }

  /**
   * 获取collection对象
   * @param {string} collectionName
   * @param {promise} callback 回调函数
   * @param {string} db
   * @returns
   */
  async collection(collectionName, callback, db) {
